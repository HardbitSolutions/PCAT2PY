...
class Finding:
    ...
    def get_rule_title(self):
        return r"The Windows SMB client will be enabled to perform SMB packet signing when possible."

    def get_vulnerability_discussion(self):
        return r"If this policy is enabled, it causes the Windows Server Message Block (SMB) client to perform SMB packet signing when communicating with an SMB server that is enabled or required to perform SMB packet signing.  This policy is defined by default in Local Computer Policy, where it is enabled by default. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ?Microsoft Network Client: Digitally sign communications (if server agrees)? to ?Enabled?."
