...
class Finding:
    ...
    def get_rule_title(self):
        return r"The Windows SMB client must be enabled to perform SMB packet signing when possible."

    def get_vulnerability_discussion(self):
        return r"The server message block (SMB) protocol provides the basis for many network operations.   If this policy is enabled, the SMB client will request packet signing when communicating with an SMB server that is enabled or required to perform SMB packet signing. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''Microsoft network client: Digitally sign communications (if server agrees)'' to ''Enabled''."
