...
class Finding:
    ...
    def get_vulnerability_discussion(self):
        return r"Inappropriate granting of user rights can provide system, administrative, and other high level capabilities.The ''Log on as a batch job'' user right allows accounts to log on using the task scheduler service, which must be restricted. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ User Rights Assignment \ ''Log on as a batch job'' to be defined but containing no entries (blank)."
