...
class Finding:
    ...
    def get_rule_title(self):
        return r"Blogging entries created from inside Office products must be configured for Sharepoint only."

    def get_vulnerability_discussion(self):
        return r"The blogging feature in Office products enables users to compose blog entries and post them to their blogs directly from Office, without using any additional software.By default, users can post blog entries to any compatible blogging service provider, including Windows Live Spaces, Blogger, a SharePoint or Community Server site, and others. If your organization has policies that govern the posting of blog entries, allowing users to access the blogging feature in Office might enable them to violate those policies. Set the policy value for: User Configuration \ Administrative Templates \ Microsoft Office 2010 \ Miscellaneous ?Control Blogging? to ?Enabled (Only SharePoint blogs allowed)?."
