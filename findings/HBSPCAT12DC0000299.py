...
class Finding:
    ...
    def get_rule_title(self):
        return r"Unauthorized remotely accessible registry paths must not be configured."

    def get_vulnerability_discussion(self):
        return r"The registry is integral to the function, security, and stability of the Windows system.  Some processes may require remote access to the registry.  This setting controls which registry paths are accessible from a remote computer.  These registry paths must be limited, as they could give unauthorized individuals access to the registry. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''Network access: Remotely accessible registry paths'' with the following entries:System\CurrentControlSet\Control\ProductOptions System\CurrentControlSet\Control\Server Applications Software\Microsoft\Windows NT\CurrentVersion"
    ...
    def check(self, cli):
        ...
        # Output Lines
        self.__output = [r'HKLM:\System\CurrentControlSet\Control\SecurePipeServers\Winreg\AllowedExactPaths', ('Machine=')] + multi_sz
