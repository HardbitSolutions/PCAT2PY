...
class Finding:
    ...
    def get_rule_title(self):
        return r"Terminal Services ? Prevent COM Port Redirection (Terminal Server Role)."

    def get_vulnerability_discussion(self):
        return r"This check verifies that the system is configured to prevent users from mapping local COM ports and redirecting data from the Terminal Server to local COM ports. Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Terminal Services \ Terminal Server \ Device and Resource Redirection ?Do not allow COM port redirection? to ?Enabled?."
