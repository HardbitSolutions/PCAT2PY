...
class Finding:
    ...
    def get_rule_title(self):
        return r"Remote Desktop Services will be configured to use session-specific temporary folders."

    def get_vulnerability_discussion(self):
        return r"This setting controls the use of per session temporary folders or of a communal temporary folder. If this setting is enabled, only one temporary folder is used for all remote desktop sessions. If a communal temporary folder is used, it might be possible for users to access other users? temporary folders. Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Remote Desktop Services \ Remote Desktop Session Host \ Temporary Folders ?Do not use temporary folders per session? to ?Disabled?."
