...
class Finding:
    ...
    def get_rule_title(self):
        return r"IPv6 source routing must be configured to the highest protection level."

    def get_vulnerability_discussion(self):
        return r"Configuring the system to disable IPv6 source routing protects against spoofing. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''MSS: (DisableIPSourceRouting IPv6) IP source routing protection level (protects against packet spoofing)'' to ''Highest protection, source routing is completely disabled''.(See ''Updating the Windows Security Options File'' in the STIG Overview document if MSS settings are not visible in the system''s policy tools.)"
    ...
    def check(self, cli):
        ...
        # Output Lines
        self.__output = [r'HKLM:\System\CurrentControlSet\Services\Tcpip6\Parameters', ('DisableIpSourceRouting=' + str(dword))]
