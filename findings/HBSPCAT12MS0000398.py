...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system must be configured to limit how often keep-alive packets are sent."

    def get_vulnerability_discussion(self):
        return r"This setting controls how often TCP sends a keep-alive packet in attempting to verify that an idle connection is still intact.  A higher value could allow an attacker to cause a denial of service with numerous connections. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''MSS: (KeepAliveTime) How often keep-alive packets are sent in milliseconds'' to ''300000 or 5 minutes (recommended)'' or less.(See ''Updating the Windows Security Options File'' in the STIG Overview document if MSS settings are not visible in the system''s policy tools.)"
