...
class Finding:
    ...
    def get_rule_title(self):
        return r"Outgoing secure channel traffic must be signed when possible."

    def get_vulnerability_discussion(self):
        return r"Requests sent on the secure channel are authenticated, and sensitive information (such as passwords) is encrypted, but the channel is not integrity checked.  If this policy is enabled, outgoing secure channel traffic will be signed. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''Domain member: Digitally sign secure channel data (when possible)'' to ''Enabled''."
