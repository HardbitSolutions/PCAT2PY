...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system must be configured to use FIPS-compliant algorithms for encryption, hashing, and signing."

    def get_vulnerability_discussion(self):
        return r"This setting ensures that the system uses algorithms that are FIPS-compliant for encryption, hashing, and signing.  FIPS-compliant algorithms meet specific standards established by the U.S. Government and must be the algorithms used for all OS encryption functions. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''System cryptography: Use FIPS compliant algorithms for encryption, hashing, and signing'' to ''Enabled''."
