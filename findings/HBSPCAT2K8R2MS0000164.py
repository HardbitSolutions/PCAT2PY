...
class Finding:
    ...
    def get_rule_title(self):
        return r"IPv6 TCP data retransmissions will be configured to prevent resources from becoming exhausted."

    def get_vulnerability_discussion(self):
        return r"This check verifies Windows is configured to control the maximum number of times that IPv6 TCP retransmits unacknowledged data segments before aborting the attempt to prevent resources from becoming exhausted. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ?MSS: (TcpMaxDataRetransmissions IPv6) How many times unacknowledged data is retransmitted (3 recommended, 5 is the default)? to ?3? or less."
    ...
    def check(self, cli):
        ...
        # Output Lines
        self.__output = [r'HKLM:\System\CurrentControlSet\Services\Tcpip6\Parameters', ('TcpMaxDataRetransmissions=' + str(dword))]
