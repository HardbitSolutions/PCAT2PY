...
class Finding:
    ...
    def get_rule_title(self):
        return r"User Account Control approval mode for the built-in Administrator must be enabled."

    def get_vulnerability_discussion(self):
        return r"User Account Control (UAC) is a security mechanism for limiting the elevation of privileges, including administrative accounts, unless authorized.  This setting configures the built-in Administrator account so that it runs in Admin Approval Mode. UAC requirements are NA on Server Core installations.Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''User Account Control: Admin Approval Mode for the Built-in Administrator account'' to ''Enabled''."
