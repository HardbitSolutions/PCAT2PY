...
class Finding:
    ...
    def get_rule_title(self):
        return r"The Windows SMB server is not enabled to perform SMB packet signing when possible."

    def get_vulnerability_discussion(self):
        return r"If this policy is enabled, it causes the Windows Server Message Block (SMB) server to perform SMB packet signing. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ?Microsoft Network Server: Digitally sign communications (if Client agrees)? to ?Enabled?."
