...
class Finding:
    ...
    def get_vulnerability_discussion(self):
        return r"Uncontrolled system updates can introduce issues to a system.  Obtaining update components from an outside source may also potentially allow sensitive information outside of the enterprise.  Application updates must be obtained from an internal source. If the \Windows\WindowsStore directory exists, Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Store \ ''Turn off Automatic Download of updates'' to ''Enabled''."
