...
class Finding:
    ...
    def get_rule_title(self):
        return r"The computer account password will not be prevented from being reset."

    def get_vulnerability_discussion(self):
        return r"Computer account passwords are changed automatically on a regular basis. Enabling this policy to disable automatic password changes can make the system more vulnerable to malicious access. Frequent password changes can be a significant safeguard for your system. If this policy is disabled, a new password for the computer account will be generated every 30 days. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ?Domain Member: Disable Machine Account Password Changes? to ?Disabled?."
