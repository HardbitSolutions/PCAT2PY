...
class Finding:
    ...
    def get_rule_title(self):
        return r"Remote Desktop Services is not configured to always prompt a client for passwords upon connection."

    def get_vulnerability_discussion(self):
        return r"This setting controls the ability of users to supply passwords automatically as part of their remote desktop connection.  Disabling this setting would allow anyone to use the stored credentials in a connection item to connect to the terminal server. Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Remote Desktop Services \ Remote Desktop Session Host \ Security ?Always prompt for password upon connection? to ?Enabled?."
