...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system will be configured to audit ''Policy Change -> Authentication Policy Change'' successes."

    def get_vulnerability_discussion(self):
        return r"Maintaining an audit trail of system activity logs can help identify configuration errors, troubleshoot service disruptions, and analyze compromises that have occurred as well as detecting attacks.  Audit logs are necessary to provide a trail of evidence in case the system or network is compromised.  Collecting this data is essential for analyzing the security of information assets and detecting signs of suspicious and unexpected behavior.Authentication Policy Change records events related to changes in authentication policy including Kerberos policy and Trust changes. Detailed auditing subcategories are configured in Security Settings \ Advanced Audit Policy Configuration.   The summary level settings under Security Settings \ Local Policies \ Audit Policy will not be enforced (see V-14230).Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Advanced Audit Policy Configuration \ System Audit Policies \ Policy Change \ ''Audit Authentication Policy Change'' with ?Success? selected."
    ...
    def check(self, cli):
        ...
        # Output Lines
        self.__output = [r'Authentication Policy Change', ('Success=' + str(enabled))]
