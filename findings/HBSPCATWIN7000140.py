...
class Finding:
    ...
    def get_rule_title(self):
        return r"Turn off indexing of mail items in Exchange Folder when Outlook is running in uncached mode."

    def get_vulnerability_discussion(self):
        return r"This check verifies that mail items on a Microsoft Exchange server are not indexed when Outlook is running in uncached mode. Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Search ?Enable indexing uncached Exchange folders? to ?Disabled?."
