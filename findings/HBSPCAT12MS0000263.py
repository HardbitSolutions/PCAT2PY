...
class Finding:
    ...
    def get_rule_title(self):
        return r"The IP-HTTPS IPv6 transition technology must be disabled."

    def get_vulnerability_discussion(self):
        return r"IPv6 transition technologies, which tunnel packets through other protocols, do not provide visibility. Set the policy value for Computer Configuration \ Administrative Templates \ Network \ TCPIP Settings \ IPv6 Transition Technologies \ ''Set IP-HTTPS State'' to ''Enabled: Disabled State''.  Note: ''IPHTTPS URL:'' must be entered in the policy even if set to Disabled State.  Enter ''about:blank''."
