...
class Finding:
    ...
    def get_rule_title(self):
        return r"Network shares that can be accessed anonymously must not be allowed."

    def get_vulnerability_discussion(self):
        return r"Anonymous access to network shares provides the potential for gaining unauthorized system access by network users.  This could lead to the exposure or corruption of sensitive data. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''Network access: Shares that can be accessed anonymously'' to be defined but containing no entries (blank)."
