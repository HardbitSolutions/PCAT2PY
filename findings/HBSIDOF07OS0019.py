...
class Finding:
    ...
    def get_rule_title(self):
        return r"Disable the Opt-In Wizard that enables first time users to opt into Internet?based Microsoft services."

    def get_vulnerability_discussion(self):
        return r"By default, the Opt-in Wizard displays the first time users run a 2007 Microsoft Office application, which allows them to opt into Internet?based services that will help improve their Office experience, such as Microsoft Update, the Customer Experience Improvement Program, Office Diagnostics, and Online Help. If your organization has policies that govern the use of such external resources, allowing users to opt in to these services might cause them to violate the policies. Set the policy value for: User Configuration \ Administrative Templates \ Microsoft Office 2007 system \ Privacy \ Trust Center ?Disable Opt-in Wizard on first run? will be set to ?Enabled?.''Note: Group Policy Administrative Templates are available from the www.microsoft.com download site. The MS Office 2007 System (Office12.adm) is included in the AdminTemplates.exe file. This template provides themechanisms to incorporate Microsoft Office 2007 System policies via the Microsoft Group Policy Editor (gpedit.msc).''''Note: If the Microsoft Group Policy Editor (gpedit.msc) is not used to incorporate the remediation to this vulnerability the Microsoft Registry Editor (regedit.exe) may be used to create the registry key and value required.''"
    ...
    def check(self, cli):
        ...
        # Output Lines
        self.__output = [r'HKCU:\Software\Policies\Microsoft\Office\12.0\Common\General', ('ShownFirstRunOptin=' + str(dword))]
