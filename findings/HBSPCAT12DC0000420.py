...
class Finding:
    ...
    def get_rule_title(self):
        return r"Only the default client printer must be redirected to the Remote Desktop Session Host.  (Remote Desktop Services Role)."

    def get_vulnerability_discussion(self):
        return r"Allowing the redirection of only the default client printer to a Remote Desktop session helps reduce possible exposure of sensitive data. Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Remote Desktop Services \ Remote Desktop Session Host \ Printer Redirection \ ''Redirect only the default client printer'' to ''Enabled''."
