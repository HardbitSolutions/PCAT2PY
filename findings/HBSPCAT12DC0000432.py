...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system must be configured to have password protection take effect within a limited time frame when the screen saver becomes active."

    def get_vulnerability_discussion(self):
        return r"Allowing more than several seconds makes the computer vulnerable to a potential attack from someone walking up to the console to attempt to log on to the system before the lock takes effect. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''MSS: (ScreenSaverGracePeriod) The time in seconds before the screen saver grace period expires (0 recommended)'' to ''5'' or less.(See ''Updating the Windows Security Options File'' in the STIG Overview document if MSS settings are not visible in the system''s policy tools.)"
