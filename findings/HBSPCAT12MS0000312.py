...
class Finding:
    ...
    def get_rule_title(self):
        return r"Windows Media Player must be configured to prevent automatic checking for updates."

    def get_vulnerability_discussion(self):
        return r"Uncontrolled system updates can introduce issues to a system.  The automatic check for updates performed by Windows Media Player must be disabled to ensure a constant platform and to prevent the introduction of unknown\untested software on the system. If Windows Media Player is installed, Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Windows Media Player \ ''Prevent Automatic Updates'' to ''Enabled''."
