...
class Finding:
    ...
    def get_rule_title(self):
        return r"The maximum age for machine account passwords must be set to requirements."

    def get_vulnerability_discussion(self):
        return r"Computer account passwords are changed automatically on a regular basis.  This setting controls the maximum password age that a machine account may have.  This setting must be set to no more than 30 days, ensuring the machine changes its password monthly. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''Domain member: Maximum machine account password age'' to ''30'' or less (excluding ''0'' which is unacceptable)."
