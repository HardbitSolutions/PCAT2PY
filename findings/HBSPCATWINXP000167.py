...
class Finding:
    ...
    def get_rule_title(self):
        return r"Terminal Services is not configured to always prompt a client for passwords upon connection."

    def get_vulnerability_discussion(self):
        return r"This setting, which is located under the Encryption and Security section of the Terminal Services configuration option, controls the ability of users to supply passwords automatically as part of their Remote Desktop Connection.  Disabling this setting would allow anyone to use the stored credentials in a connection item to connect to the terminal server. Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Terminal Services \ Encryption and Security ?Always Prompt Client for Password upon Connection?  to ?Enabled?."
