...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system is not configured to meet the minimum requirement for session security for NTLM SSP based Servers."

    def get_vulnerability_discussion(self):
        return r"Microsoft has implemented a variety of security support providers for use with RPC sessions.  In a homogenous Windows environment, all of the options should be enabled and testing should be performed in a heterogeneous environment to determine the maximum-security level that provides reliable functionality. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ?Network security: Minimum session security for NTLM SSP based (including secure RPC) servers? to ?Require NTLMv2 session security?, ?Require 128-bit encryption (all options selected)."
    ...
    def check(self, cli):
        ...
        # Output Lines
        self.__output = [r'HKLM:\System\CurrentControlSet\Control\Lsa\MSV1_0', ('NTLMMinServerSec=' + str(dword))]
