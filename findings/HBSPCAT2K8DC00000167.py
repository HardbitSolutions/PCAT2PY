...
class Finding:
    ...
    def get_rule_title(self):
        return r"Terminal Services ? Remove Disconnect Option from Shut Down dialog box (Terminal Server Role)."

    def get_vulnerability_discussion(self):
        return r"This check verifies that the system is configured to remove the Disconnect option from the Shut Down Windows dialog box on the client. 2008 - Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Terminal Services \ Terminal Server \ Remote Session Environment ?Remove ?Disconnect? option from the Shut Down dialog? will be set to ?Enabled?."
