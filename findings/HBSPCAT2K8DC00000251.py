...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system does not generate an audit event when the audit log reaches a percent full threshold."

    def get_vulnerability_discussion(self):
        return r"When the audit log reaches a given percent full, an audit event is written to the security log. The event ID is 523 and is recorded as a success audit under the category of System. This option may be especially useful if the audit logs are set to be cleared manually. A recommended setting would be 90 percent. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ?MSS: (WarningLevel) Percentage threshold for the security event log at which the system will generate a warning? to ?90? or less."
