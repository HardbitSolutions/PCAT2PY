...
class Finding:
    ...
    def get_rule_title(self):
        return r"Terminal Services / Remote Desktop Services - Local drives prevented from sharing with Terminal Servers (Terminal Server Role)."

    def get_vulnerability_discussion(self):
        return r"This check verifies that the system is configured to prevent users from sharing the local drives on their client computers to Terminal Servers that they access. Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Terminal Services \ Terminal Server \ Device and Resource Redirection ?Do not allow drive redirection? to ?Enabled?."
