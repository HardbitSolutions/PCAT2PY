...
class Finding:
    ...
    def get_rule_title(self):
        return r"PKI certificates associated with user accounts must be issued by the DoD PKI or an approved External Certificate Authority (ECA)."

    def get_vulnerability_discussion(self):
        return r"A PKI implementation depends on the practices established by the Certificate Authority (CA) to ensure the implementation is secure.  Without proper practices, the certificates issued by a CA have limited value in authentication functions. Replace unauthorized certificates associated with user accounts with ones issued by the DoD PKI or an approved External Certificate Authority."
