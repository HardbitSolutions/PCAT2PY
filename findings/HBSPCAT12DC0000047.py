...
class Finding:
    ...
    def get_rule_title(self):
        return r"The computer account password must not be prevented from being reset."

    def get_vulnerability_discussion(self):
        return r"Computer account passwords are changed automatically on a regular basis.  Disabling automatic password changes can make the system more vulnerable to malicious access.  Frequent password changes can be a significant safeguard for your system.  A new password for the computer account will be generated every 30 days. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''Domain member: Disable machine account password changes'' to ''Disabled''."
