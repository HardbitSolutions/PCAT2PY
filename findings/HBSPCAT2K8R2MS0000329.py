...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system will limit how many times unacknowledged TCP data is retransmitted."

    def get_vulnerability_discussion(self):
        return r"In a SYN flood attack, the attacker sends a continuous stream of SYN packets to a server, and the server leaves the half-open connections open until it is overwhelmed and no longer able to respond to legitimate requests. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ?MSS: (TcpMaxDataRetransmissions) How many times unacknowledged data is retransmitted (3 recommended, 5 is the default)? to ?3? or less."
