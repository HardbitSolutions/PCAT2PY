...
class Finding:
    ...
    def get_rule_title(self):
        return r"Launching programs and unsafe files property is properly set (Internet Zone)."

    def get_vulnerability_discussion(self):
        return r"This policy setting controls whether or not the ?Open File ? Security Warning? prompt is shown when launching executables or other unsafe files.  If you do not configure this policy setting, users can configure the prompt behavior.  By default, execution is blocked in the Restricted Zone, enabled in the Intranet and Local Computer Zone, and set to prompt in the Internet and Trusted Zones. Manipulate the value: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3 Criteria: Set the value 1806 to REG_DWORD = 1."
