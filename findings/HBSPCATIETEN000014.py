...
class Finding:
    ...
    def get_rule_title(self):
        return r"Navigating windows and frames across different domains must be disallowed (Internet zone)."

    def get_vulnerability_discussion(self):
        return r"Frames that navigate across different domains are a security concern, because the user may think they are accessing pages on one site while they are actually accessing pages on another site. It is possible that a website hosting malicious content could use this feature in a manner similar to cross site scripting. This policy setting allows you to manage the opening of sub-frames and access of applications across different domains. Manipulate the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> ''Navigate windows and frames across different domains'' to ''Enabled'', and select ''Disable'' from the drop-down box."
