...
class Finding:
    ...
    def get_rule_title(self):
        return r"Windows must be prevented from sending an error report when a device driver requests additional software during installation."

    def get_vulnerability_discussion(self):
        return r"Some features may communicate with the vendor, sending system information or downloading data or components for the feature.  Turning off this capability will prevent potentially sensitive information from being sent outside the enterprise and uncontrolled updates to the system.  This setting will prevent Windows from sending an error report to Microsoft when a device driver requests additional software during installation. Set the policy value for Computer Configuration \ Administrative Templates \ System \ Device Installation \ ''Prevent Windows from sending an error report when a device driver requests additional software during installation'' to ''Enabled''."
    ...
    def check(self, cli):
        ...
        # Output Lines
        self.__output = [r'HKLM:\Software\Policies\Microsoft\Windows\DeviceInstall\Settings', ('DisableSendRequestAdditionalSoftwareToWER=' + str(dword))]
