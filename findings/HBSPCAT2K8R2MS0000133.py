...
class Finding:
    ...
    def get_rule_title(self):
        return r"The user will be prompted for a password on resume from sleep (Plugged In).  (Applicable on Server 2008 R2 if the system is configured to sleep.)"

    def get_vulnerability_discussion(self):
        return r"This check verifies that the user is prompted for a password on resume from sleep (Plugged In). Set the policy value for Computer Configuration \ Administrative Templates \ System \ Power Management \ Sleep Settings ?Require a Password When a Computer Wakes (Plugged In)? to ?Enabled?."
