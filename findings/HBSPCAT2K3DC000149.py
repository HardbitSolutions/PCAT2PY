...
class Finding:
    ...
    def get_rule_title(self):
        return r"Terminal Services is not configured to limit users to one remote session."

    def get_vulnerability_discussion(self):
        return r"This setting limits users to one remote session.  It is possible, if this setting is disabled, for users to establish multiple sessions. 2003 - Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Terminal Services ?Restrict Terminal Server users to a Single Remote Session? to ?Enabled?."
