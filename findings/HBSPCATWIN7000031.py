...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system configuration is not set with a password-protected screen saver."

    def get_vulnerability_discussion(self):
        return r"The system should be locked when unattended.  Unattended systems are susceptible to unauthorized use.  The screen saver should be set at a maximum of 15 minutes and be password-protected.  This protects critical and sensitive data from exposure to unauthorized personnel with physical access to the computer. Windows 7 - Set the policy values for User Configuration \ Administrative Templates \ Control Panel \ Personalization \ as follows:?Enable Screen Saver? will be set to ?Enabled?.?Password protect the screen saver? will be set to ?Enabled?.?Screen Saver timeout? will be set to ?Enabled: 900 seconds? (or less)."
    ...
    def check(self, cli):
        ...
        # Output Lines
        self.__output = [r'HKCU:\Software\Policies\Microsoft\Windows\Control Panel\Desktop', ('ScreenSaveActive=' + sz)]
