...
class Finding:
    ...
    def get_rule_title(self):
        return r"Remote Desktop Services will limit users to one remote session."

    def get_vulnerability_discussion(self):
        return r"This setting limits users to one remote session.  It is possible, if this setting is disabled, for users to establish multiple sessions. Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Remote Desktop Services \ Remote Desktop Session Host \ Connections ?Restrict Remote Desktop Services users to a Single Remote Desktop Services Session? to ?Enabled?."
