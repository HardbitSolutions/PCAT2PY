...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system must be configured to prevent the storage of the LAN Manager hash of passwords."

    def get_vulnerability_discussion(self):
        return r"The LAN Manager hash uses a weak encryption algorithm and there are several tools available that use this hash to retrieve account passwords.  This setting controls whether or not a LAN Manager hash of the password is stored in the SAM the next time the password is changed. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''Network security: Do not store LAN Manager hash value on next password change'' to ''Enabled''."
