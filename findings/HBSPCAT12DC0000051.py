...
class Finding:
    ...
    def get_rule_title(self):
        return r"Users must be warned in advance of their passwords expiring."

    def get_vulnerability_discussion(self):
        return r"Creating strong passwords that can be remembered by users requires some thought.  By giving the user advance warning, the user has time to construct a sufficiently strong password.  This setting configures the system to display a warning to users telling them how many days are left before their password expires. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''Interactive Logon: Prompt user to change password before expiration'' to ''14'' days or more."
