...
class Finding:
    ...
    def get_rule_title(self):
        return r"Unauthorized accounts must not have the Bypass traverse checking user right."

    def get_vulnerability_discussion(self):
        return r"Inappropriate granting of user rights can provide system, administrative, and other high-level capabilities.Accounts with the ''Bypass traverse checking'' user right can pass through folders when browsing even if they do not have the ''Traverse Folder'' access permission. They could potentially view sensitive file and folder names.  They would not have additional access to the files and folders unless it is granted through permissions. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ User Rights Assignment \ ''Bypass traverse checking'' to only include the following accounts or groups:AdministratorsAuthenticated UsersLocal ServiceNetwork ServiceWindow Manager\Window Manager Group"
