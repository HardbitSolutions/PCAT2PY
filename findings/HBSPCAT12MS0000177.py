...
class Finding:
    ...
    def get_rule_title(self):
        return r"IPv6 TCP data retransmissions must be configured to prevent resources from becoming exhausted."

    def get_vulnerability_discussion(self):
        return r"Configuring Windows to limit the number of times that IPv6 TCP retransmits unacknowledged data segments before aborting the attempt helps prevent resources from becoming exhausted. Set the policy value for Computer Configuration \ Windows Settings \ Security Settings \ Local Policies \ Security Options \ ''MSS: (TcpMaxDataRetransmissions IPv6) How many times unacknowledged data is retransmitted (3 recommended, 5 is the default)'' to ''3'' or less.(See ''Updating the Windows Security Options File'' in the STIG Overview document if MSS settings are not visible in the system''s policy tools.)"
