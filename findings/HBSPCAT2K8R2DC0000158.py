...
class Finding:
    ...
    def get_rule_title(self):
        return r"The system will be configured to remove the Disconnect option from the Shut Down Windows dialog box on the Remote Desktop Client. (Remote Desktop Services Role)"

    def get_vulnerability_discussion(self):
        return r"This check verifies that the system is configured to remove the Disconnect option from the Shut Down Windows dialog box on the client. Set the policy value for Computer Configuration \ Administrative Templates \ Windows Components \ Remote Desktop Services \ Remote Desktop Session Host \ Remote Session Environment ?Remove ''Disconnect'' option from Shut Down dialog? to ?Enabled?."
